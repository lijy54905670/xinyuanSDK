        ...
        try {
            ...
            switch (lCommand) {
                ...
                case 0x1102:	//交通抓拍的终端图片上传
                    ...
                    HCNetSDK.NET_VCA_RULE_ALARM ss = new HCNetSDK.NET_VCA_RULE_ALARM();

                    ss.write();
                    Pointer pointer = ss.getPointer();
                    pointer.write(0,pAlarmInfo.getByteArray(0,ss.size()),0,ss.size());
                    ss.read();


                    int dwAbsTime = ss.dwAbsTime;
                    int dwPicDataLen = ss.dwPicDataLen;
                    SimpleDateFormat format =  new SimpleDateFormat( "yyyy-MM-dd HH:mm:ss" );
                    String d = format.format(dwAbsTime);
                    short wEventTypeEx = ss.struRuleInfo.wEventTypeEx;
